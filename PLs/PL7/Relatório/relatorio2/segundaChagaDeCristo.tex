...
\begin{document} 
	...
	\section{Introdução teórica}
		...
		\begin{equation}
			\lambda = \frac{v_{US}}{f_{US}} \approx 3.75/1.88 \,\unit{\mm}
			\label{eq:lambda}
		\end{equation}
	...
	\section{Análise de dados}
		...
		Assim tem-se dados de amplitude do envelope de onda em função da distância percorrida ao longo do material. A relevância deste procedimento é que, deste modo, consegue-se estudar a atenuação da onda ultra-sónica nas dimensões de comprimento, como seria de esperar.
		\par
		Primeiramente, usou-se uma função de inverso do ganho (ver código em .---) para se obter o sinal de tensão variável proveniente do transdutor \textbf{sem efeitos adicionais}, uma vez que será a partir daí que se poderá concluir um decréscimo de intensidade de onda. \\
		...
		\par
		O ajuste exponencial, em particular, será do tipo 
		\begin{equation}
	...
	\subsection{Segunda parte}
		...
		\begin{table}[h!]
			...
			\begin{tabular}{c|cccc}
				...
				\bottomrule
			\end{tabular}
			\caption{Profundidades (em \unit{mm}) de cada defeito A,B,C na orientação 1 e 2 em função da frequência de sonda ultra-sónica. Assume-se que cada valor destes tem incerteza estimada de $\SI{0.1}{\mm}$, a menor divisão da escala.}
			\label{tab:profs}
		\end{table}

		Pela expressão \ref{eq:diams} retiraram-se os diâmetros estimados para A, B (tab. \ref{tab:diamsAB}). Uma vez que C é uma junção de duas pequenas imperfeições, desta forma não se iria distingui-las, pelo que uma análise mais minuciosa será feita.\\
		Como os diâmetros foram calculados por uma soma/subtração de distâncias com respetivas incertezas, as suas incertezas foram estimadas por propagação de incertezas (na secção \ref{A:incDiam}).
	...
	\section{Discussão de resultados}
		...
		O cálculo da atenuação consistiu no ajuste exponencial de um conjunto de pontos (relembra-se os parâmetros e incertezas da tabela \ref{tab:atenu}). \\
		...
		\begin{table}[H]
			...
			\begin{tabular}{|c|c|c|}
				...
				\bottomrule
			\end{tabular}
			\caption{Coeficiente de absorção em unidades de \unit{\dB\,\cm^{-1}}.}
			\label{tab:atenuDeciBel}
		\end{table}
		
		Seria de esperar que o coeficiente de absorção no acrílico aumentasse com a frequência (expectavelmente há mais interação com o meio), no entanto os resultados obtidas na tabela \ref{tab:atenuDeciBel} não concordam com este facto. Denota-se que as incertezas de cada coeficiente são comparáveis à diferença entre os dois valores $\mu$ obtidos, pelo que compará-los poderá não ser correto à partida.
